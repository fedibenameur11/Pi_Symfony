...
class SalleRepository extends ServiceEntityRepository
{
    ...
    public function findByAnything($value)
    {
        return $this->createQueryBuilder('S')
            ->where("S.nom_salle LIKE '%$value%'
            OR S.adresse_salle LIKE '%$value%'
            OR S.num_telephone LIKE '%$value%'
            OR S.codepostal LIKE '%$value%'
            OR S.ville LIKE '%$value%'
            OR S.prix_abonnement LIKE '%$value%'")
            ->getQuery()
            ->getResult();
    }
